...
function GeneroPorNombre() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `http://localhost:8068/operaciongettipogeneronombretipogenero/${encodeURIComponent(nombre)}`
      );
      if (!response.ok) {
  ...
  return (
    <div className='genero-container'>
      <h1 className='genero-title'>Consultar Tipo Género por Nombre</h1>
