...
function GeneroTodos() {
  ...
  const jsonUrl = 'http://localhost:8068/operaciongetcomprador'; // Asegúrate de que esta sea la URL correcta
  ...
  return (
    <div className='genero-container'>
      <h1 className='genero-title'>Listado de Compradores</h1>
      <table className='genero-table'>
        ...
        <tbody>
          {data.map((row) => (
            <tr key={row.idcomprador}>
              <td>{row.idcomprador}</td>
              <td>{row.nombre}</td>
              <td>{row.apellido}</td>
