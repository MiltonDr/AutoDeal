...
import './NameTipoEstadoCivil.css';
...
function GeneroPorNombre() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `http://localhost:8068/operaciongettipoestadocivilnombretipoestadocivil/${encodeURIComponent(nombre)}`
      );
      if (!response.ok) {
  ...
  return (
    <div className='genero-container'>
      <h1 className='genero-title'>Consultar Tipo Estado Civil por Nombre</h1>
      <form onSubmit={handleSubmit}>
        <input
          type='text'
          placeholder='Ingresa el nombre del estado civil'
          value={nombre}
